...
## 9. 이 계약이 낳은 것 — 닫힘 (2026-09-01)

어휘가 고정되자 각 행이 **언제 · 얼마나 오래** 떠 있는지가 확정됐고, 그것이 곧 작화
기준이 됐다. v3 시트가 그 기준으로 만들어졌다 — 14개 트랙, 표준 57칸 + 확장 19칸,
**지면선은 모든 행에서 175로 통일**. 지금 무엇이 그려져 있는지는 `docs/art/mochi-sheet.md`.

계약이 실제로 바꾼 것 셋:

- `waving`은 0초에서 **완료 축하**로 바뀌었다.
- `review`는 `observe` 전용이 되었고 1.03초 뒤 물러난다. 커서 응시는 `gaze`로 빠져나갔다 —
  전용 행을 네 번 시도했지만 고양이가 커져 버려서, 같은 여섯 칸을 **재생 속도만 달리해**
  빌리는 것으로 정해졌다(커서가 가까울수록 두 배).
- `jumping`의 시작·끝 프레임이 지면선보다 25px 아래이던 결함은 **v3에서 없어졌다.** 이 행은
  `spark`(턴 시작)와 `landing`(착지) 둘 다에 쓰여서 가장 자주 보인다.
