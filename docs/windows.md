...
- `rust/roamling-win/src/platform.rs` — 키보드만의 idle. **`GetAsyncKeyState`를 폴링한다
  ...
  **탐색키도 타이핑으로 센다 — 읽는 것도 일하는 것이다 (사용자 결정 2026-09-14).**
  `is_typing_key`에 PageUp·PageDown·Home·End·방향키가 들어 있다. 한글·한쇼·한셀은 **편집**
  도구라 방향키와 Home·End는 읽기가 아니라 글자 사이를 옮기는 동작이고, 이걸 빼면 고치는
  사이사이 커서만 옮기는 구간에서 갸웃이 잘못 뜬다.

  **대가는 기기에 따라 답이 갈리는 것이다.** 같은 문서를 PageDown으로 읽으면 계속 `Active`,
  마우스 휠로 읽으면 10초 뒤 갸웃이다. 반대로 맞출 수가 없어서 받아들인 것이다 — 휠은 가상
  키가 없어 `GetAsyncKeyState`로 못 읽고, 저수준 마우스 훅은 위의 금지 규칙에 걸린다.
  **이 비대칭이 결함으로 다시 올라오면 그때는 "휠을 세자"가 아니라 "탐색키를 빼자"가 답이다.**
