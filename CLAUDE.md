...
### G. agent 없이도 "일하는 중"을 안다 — 2026-09-11 macOS 구현, 실사용 확인 대기 · Windows 배선 대기 (W8)
...
```
일하는 앱이 앞에 옴           → 옆으로 와서 그냥 앉아 있음(idle)       [present]
그 세션의 첫 타이핑           → 점프, 이어서 running                  [activityStarted → highIntensity]
세션 안의 다음 타이핑         → 즉시 running                          [highIntensity]
마지막 키 뒤 10초             → 갸웃                                 [attentionRequired]
갸웃 5초                      → 자리를 비우고 원래대로 돌아다님         [activityEnded]
돌아다니다 다시 타이핑         → 걸어와서 running, 점프 없음
타이핑 없이 보기만 함          → 옆에 계속 앉아 있음                    [present 60초 재발신]
앱을 3초 이상 떠남            → 끝. 이번 세션에서 3분 이상 쳤으면 먼저 손 흔듦(agent가 자리를 지키면 안 흔듦)
...
누른 Cmd-Tab도 키 입력이다** — 그 앱이 앞에 온 뒤에 눌린 키만 타이핑으로 센다.

**이벤트 종류가 하나 늘었다: `present`.** 기존 kind는 전부 도착하면 무언가를 한다(점프·갸웃·
running, `inspecting`은 창 힌트를 안 받아 걸어오지도 않는다). 게다가 director는 dispatch마다
도착 반응을 다시 걸고 앉아 있는 펫에게 그대로 입히므로, 자리를 지키려는 60초 재발신이 그 반응을
분마다 재생한다. 재생해도 안 보이는 것은 Calm(idle)뿐이라 `present`는 Calm으로 앉는다. 뜻은
"사용자가 자기 일 앞에 있고, 반응할 것은 없다"이고 agent는 내지 않는다. Swift 원본과 Rust
양쪽에 있고 `CoreLogicTests`의 Swift↔Rust 대조가 둘을 비교한다. 자는 펫을 깨우지 않는다.
attention에서는 좌석을 잡는 kind 중 최하위다.

**규칙은 core, 접촉은 셸.** `rust/roamling-core/src/focus_activity.rs`의 상태 기계가 전부이고
결정적(`now` 인자)이다. 상수는 전부 그 파일 상단에 있다. 셸은 0.5초마다 기계와 설정에서 셋(앞에
있는 앱 id, 그 앱이 일하는 앱인지, 마지막 키 입력 뒤 초)과 **펫의 사실 넷**을 넘긴다:

- **방금 처리한 이벤트 id + 도착 반응 대기** — 인사 점프의 박자. 점프 뒤 running을 너무 일찍
  보내면 걸어오는 동안 점프를 덮는다. 처음엔 "펫이 누구 자리를 지키나"로 판정했는데, 점프가
  **이미 우리 자리에 앉아 있을 때** 나가는 흐름에서는 그게 보내기 전부터 참이었다.
- **쉬는 중(`is_resting`)** — director는 안 깨우는 kind가 자는 펫에게 오면 **미루지 않고 버린다.**
  펫을 깨우는 것은 키 입력인데 그 판정이 한 tick 안에서 샘플보다 뒤에 돌아, 졸던 펫 옆에서 첫
  타이핑을 하면 점프가 버려졌다. 그래서 source는 펫이 쉬는 동안 자리 소식을 보내지 않고 깬 첫
  샘플에 그때 상황에 맞는 것을 보낸다. director와 **같은 술어**라 보냈는데 버려지는 일이 구조적으로
  없다. 손 흔들기는 쉬는 중에도 바로 간다(흔든 뒤의 자리 비움은 아래 손 흔들기 규칙을 따른다).

- **agent가 자리를 지키는 중(`agent_on_duty`)** — 떠날 때 손 흔들기를 보낼지. director의 판단을
  `PetRuntime`·`PetLoop`·`RustPetLoop` 통로로 그대로 읽는다.

**`roamling.workApps`는 첫 리스트형 설정이다** — 쉼표 구분 문자열, 비면 키를 지운다(튜닝과
같은 규칙). macOS는 번들 id, Windows는 exe 이름. 메뉴 "일하는 앱"에 **최근 앞에 있었던
앱**이 체크박스로 뜬다(크롬도 체크 안 된 채로 떠서 켤 수 있다). 기본값은 macOS 빈 목록이다.
Windows 기본값은 W8을 구현할 때 그 기계에서 실제 exe 이름을 확인해 정한다(한컴오피스 추정:
`Hwp.exe` · `Hshow.exe` · `Hcell.exe`).

**agent와 같이 있을 때는 agent가 우선이다 (사용자 결정 2026-09-11).** agent가 자리를 지키는 동안 —
agent 신호가 30초 안에 왔거나, agent가 자리를 쥐고 5분 침묵 만료 전 — 일하는 앱 신호는 **점수와
무관하게 후보에서 빠진다.** director가 attention에 후보를 넘기기 전에 거르므로 attention의 점수·이력
여유·dwell·쿨다운은 그대로다. 그래서 Claude가 일하는 동안 한글에서 타이핑하거나 멈춰도 펫은 Claude
옆에 남고, Claude가 끝나면(Stop·ActivityEnded) 곧바로, 5분 침묵 만료로 자리가 비었으면 다음 재발신(최대 15초) 때 일하는 앱이 이어받는다. 30초 넘은 신호는 후보가 아니므로
일하는 앱 source는 막혀 있는 동안 15초마다 지금 상태를 다시 보낸다. 인사 점프는 자리를 실제로 받을
때까지 아껴 둔다.

director가 둘을 구분해야 해서 **Rust `CompanionEvent`에 출처 종류(`source_type`)가 생겼다** — Swift
`ActivitySourceType`을 옮긴 것이다. 생성자 기본값이 Agent라 Windows 셸·agent 정규화·픽스처는 안
바뀌었고, 일하는 앱 이벤트만 System이다. 전에는 Swift→Rust 변환이 이 값을 버렸다.
...
닫기 때문에, 한때 "펫이 그 앱 옆에 있을 때만"으로 두었더니 손 흔들기가 거의 나오지 않았다.

**Claude가 자리를 지키고 있으면 손 흔들기를 아예 보내지 않는다**(`agent_on_duty`). 보내고 B 규칙이
거르게 두었더니, 걸러진 손 흔들기가 대기 후보에 남아 있다가 몇 초 안에 Claude가 끝나면 그 축하 직후
**늦게 튀어나왔다**(보통 짧은 Glance) — 테스트에서 Claude가 끝난 1초 뒤 쳐다보는 것으로 재현했다. 옛
판단 방식("director가 마지막으로 처리한 이벤트가 우리 것인가")은 Claude가 끝난 뒤에도 그 id가 남아 흔들기를
잘못 막았으므로 쓰지 않는다.

흔들었으면 자리 비움은 **손 흔들기가 dispatch된 뒤** 보내고 5초 상한(`WAVE_HOLD_TIMEOUT`)을 둔다. 쉬는
펫에게 온 손 흔들기는 대기열에 들어가 펫이 Idle이 된 뒤(깬 뒤 약 1.7초)에 나가는데, 그 전에 자리 비움이
오면 대기열을 지웠다. 실기에서는 키 입력이 펫을 깨우므로 거의 생기지 않는 경우라 보험이다. 5초 상한은 손 흔들기를 보낸 뒤 agent가 자리를 잡은 경우에 손 흔들기가 늦게 나올 수 있는 시간의 상한이기도 하다.

**앱을 바꾸느라 누른 키는 갸웃 10초를 다시 시작시키지 않는다.** 앱이 앞에 온 뒤에 눌린 키만 센
시각을 따로 들고(`last_counted_key_at`) 갸웃 판단에 쓴다.
...
**다음 할 일(사용자가 진행을 말하면): 흐름 문서.** 파이프라인 한 장, 일하는 앱 상태 기계, director에서
발목 잡는 동작과 그것이 낳은 우회 코드, 시나리오별 초 단위 시간표, 우회 코드를 director 개념으로
흡수하는 리팩터링 후보 — 위 "작업 방식" 규칙대로 문장마다 코드 위치를 단다.

남은 것: Windows 셸 배선(`docs/windows.md` W8 — 프로세스 이름, 훅 없는 키보드 판정, 설정,
트레이 하위 메뉴, 그리고 위 일곱 입력). 흐름과 시간표는 `docs/behavior-flow.md` 5b절.
