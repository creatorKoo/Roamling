...
`begin_catch`로 들어간다. 빠른 접근이나 0.35초 제한은 클릭의 전제 조건이 아니다. 커서를
올리는 것만으로 걷기나 작업을 멈추지는 않는다. 클릭 후 짧은 반응을 재생하는 동안에는 기존처럼
클릭을 통과시키며, 드래그 중에는 몸체 밖에서도 버튼을 놓을 때까지 입력을 소유한다.
기존 빠른 접근 판정(74pt, 380pt/s, 접근 182.4pt/s)과 0.35초 창은 응시가 가능한 상태의
접근 반응·틱 속도에만 남는다(`PointerInteractionModel::evaluate`, `preferred_tick_interval`).
...
**일반 응시는 가만히 앉아 있을 때만 한다 (2026-09-17).** `Idle`·`Sit`·`LookAtPointer`이며
경로가 없고 속도가 1pt/s 이하일 때만 커서를 보고 꼬리를 흔든다. 배회·목적지 이동·휴식 장소
이동 중에는 커서의 `Watching`·`Catchable`이 걸음을 멈추지 않고, 도착해 앉으면 다시 반응한다.
작업·알림·점프 등 별도 반응도 일반 응시로 덮지 않는다. 근거: `PetRuntime::finish_tick`의
`allows_pointer_glance`. 애정 키와 실제 잡기는 아래의 명시적 상호작용으로 유지한다.

**앉아 응시하던 중에도 사용자 글자 위에서 벗어나는 걸음은 170px 대역을 무시한다.**
...
된다. 100px 안쪽의 회피와 실제 잡기는 계속 우선한다. 단, 공유 모니터 경계 통과 중에는
안쪽까지 이동한 뒤 회피를 재개하고 실제 잡기만 즉시 받는다.
근거는 `docs/placement.md`의 모니터 경계 규칙과 3.2.2.
...
키나 접근 속도와 무관하게 몸체 클릭으로 받는다. 키를 떼면 그 tick부터 커서
...
**좌석은 커서에서 인식 거리(기본 170px) 밖에서 고른다.** 기존에는 흐름 B의 응시가 걸음을
멈춰 170px 앞에서 서고 출발하기를 반복했다. 지금은 이동 중 응시를 생략하지만, 커서에
가려진 곳을 목적지로 고르지 않는 배치 규칙은 유지한다. 걷는 중에 커서가 목적지에 앉으면
...
**작업 중 커서 응시 (2026-09-17).** source의 지속 반응이 `work`이거나 현재 상태가
`Work`이면, 일반 마우스의 `Watching`·`Catchable`이 작업을 꼬리 흔들기로 바꾸지 않는다.
잡기 대기 중에도 작업을 유지하고 실제 클릭은 그대로 받는다. 가까운 커서 회피와 애정 키로
명시적으로 쓰다듬는 동작은 유지하며, 그 뒤에도 source가 작업 중이면 작업 모션으로 돌아온다.
창 위치를 못 찾은 source에도 같다. 일반 배회·목적지 이동의 응시 억제는 흐름 B의
앉은 상태 조건으로 별도 적용한다.
근거: `PetRuntime::finish_tick`, `make_situation`, `apply_intent`,
`ActivityDirector::sustained_reaction`.
...
**승인이 났다는 훅은 없다.** 승인 뒤에 오는 것은 그 도구가 끝난 뒤의 `PostToolUse`뿐이고, 그것은 평소에는
버려지는 일상 진행이다(`Positive` 0.08 < `ROUTINE_POSITIVE_INTENSITY`). 그대로 두면 갸웃은 다음 도구의
`PreToolUse`까지 — 승인받은 명령이 도는 내내, 그리고 그 뒤로도 — 이어진다. 그래서 **승인 대기를 입고 있는
agent에게서 온 일상 진행은 "물어본 도구가 돌았다 = 답을 받았다"로 읽고 `work`로 돌린다**
(`ActivityDirector::handle_event`, `resuming_after_approval`; 포팅 대조군과 비교되는 기본값에서는 꺼져 있고
런타임이 켠다). 도구가 끝나야 알 수 있으므로 긴 명령이 도는 동안의 갸웃은 남는다 — `docs/requests.md` B8.
...
입력**에, 갸웃은 **타이핑이 멈췄을 때**만 나온다. 갸웃도 10초(`WAITING_BEFORE_RELEASE`)면 자리를
...
기본값은 **채워져 있다** — Windows는 한컴 셋(`Hwp.exe` · `HShow.exe` · `HCell.exe`)과 MS Office
셋(`WINWORD.EXE` · `POWERPNT.EXE` · `EXCEL.EXE`), macOS는 MS Office 셋뿐이다(맥에서는 한글·한쇼를
쓰지 않는다). 처음 실행부터 이 흐름이 돈다. **켜져 있는 것만으로 성가셔지지 않는 이유는 위에
있다** — 앱이 앞에 와도 옆에 앉기만 하고, 점프는 첫 키 입력에, 갸웃은 타이핑이 멈춰야 나온다.
메뉴에서 전부 끈 빈 목록은 명시적으로 저장되어 다음 실행에 기본값이 되살아나지 않는다.
...
| 지정 앱 갸웃 | **10초** (사용자 결정 2026-09-12) | 키 입력 → 타이핑, 아니면 자리 해제 | `focus_activity.rs` `WAITING_BEFORE_RELEASE` |
