...
### E. 저장된 설정이 옛 기본값을 얼린다 — 아직 남아 있다

A2가 Windows에 닿지 않아서 드러났다. **저장된 값이 기본값을 이기는데, 패널을 한 번이라도
열면 열한 값이 전부 저장된다** — 그래서 걷기 사이 멈춤을 40으로 올려도 파일에 12가 박힌
기계에는 영영 오지 않는다. 실제로 그랬다. "기본값으로"를 누른 것이 그 파일을 만든 원인이라,
가장 성실한 사용자가 가장 확실하게 갇힌다.

Windows는 **기본값과 같은 값을 저장하지 않고 지우도록** 고쳤다. 안 건드린 항목은 기본값을
계속 따라가고, 하나를 옮겨도 나머지 열을 붙잡지 않고, "기본값으로"는 키를 하나도 남기지
않는다.

**macOS는 아직이다.** `RoamlingRuntime`이 `roamling.runtimeTuning`에 JSON 한 덩어리를
넣으므로 그대로는 같은 처리가 안 된다 — 기본값과 다른 항목만 담거나, 전부 기본값이면 키를
지우는 쪽으로 바꿔야 한다. 이걸 안 하면 다음 기본값 변경도 맥 사용자에게 안 간다.
