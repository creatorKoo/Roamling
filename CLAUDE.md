...
### G. agent 없이도 "일하는 중"을 안다 — macOS 확인됨 2026-09-12 · Windows 실사용 확인 대기 (W8)
...
- **기본값은 채워져 있다 (사용자 결정 2026-09-12).** Windows는 한컴 셋과 MS Office 셋,
  macOS는 MS Office 셋뿐이다 — 맥에서는 한글·한쇼를 쓰지 않는다. 목록은 `docs/windows.md` W8.
  **켜져 있어도 성가시지 않은 이유는 구조에 있다** — 지정 앱이 앞에 와도 펫은 옆에 앉기만 하고,
  점프는 그 세션의 첫 키 입력에만, 갸웃은 타이핑이 멈춰야 나온다.
- **빈 목록도 저장한다.** 기본값이 생긴 순간 "비면 키를 지운다"는 "전부 껐다"를 "기본값으로
  되돌려라"로 바꿔 버린다 — 끌 수 없는 기능이 된다. 키가 아예 없을 때만 기본값이 답한다.
