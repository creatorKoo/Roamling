...
여전히 row 6 하나가 다섯 상태(승인 대기·앉기·잠·잡힘·끌림)를 감당하고, `gaze`·`stretch`는
idle과 같은 그림이다. 그 둘이 다음에 그릴 대상이다.
...
idle (row 0, 앉아서 깜박)
 ...
 │                                    └─ 목적지 도착 ──▶ idle
 │
 └─ 사용자 입력이 75초 없음 ────▶ sit (row 6, 앉아서 위 보기)
     + 커서가 170px 밖                │
     + 이동 중 아님                   └─ 2.4초 뒤 ──▶ findSleepSpot (row 1/2, 0.75배속 걷기)
                                                        │  안전지대까지 걸어감
                                                        │  (에이전트 옆 좋은 자리면 제자리)
                                                        └─ 도착 ──▶ sleep (row 6, sit과 같은 그림)
                                                                     │
...
- **`gaze`와 `stretch`는 그릴 그림이 없다.** 커서가 다가올 때와 기지개가 idle과 같다.
