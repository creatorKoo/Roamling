...
주요 조작·기능이 바뀌면 배포 전에 [사용 안내 절차](usage-guide.md)의 안내 revision과 한영 문구도
함께 갱신한다. 오류·성능 수정만 있는 릴리스는 안내 revision을 올리지 않는다.
`ROAMLING_SMOKE_TEST=1`에서는 안내를 표시하거나 읽음 상태를 저장하지 않는다.
