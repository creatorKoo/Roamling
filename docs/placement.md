...
2026-09-10까지 10번은 무작위 후보 6개 중 **emptiness 0.55를 처음 넘는 것**을 그대로
골랐다. 점수는 펫 자기 프레임(96×104)만 재므로, 문단에서 1px 떨어진 자리와 바탕화면
한가운데가 똑같이 만점이었다 — 둘을 구분할 값이 아예 없었다. 그리고 후보가 화면 아래
170px 띠에 72% 몰린 무작위 6개뿐이라, 화면에 하나뿐인 빈 구역에 후보가 떨어질 보장도
없었다. 6개가 전부 글자 위면 가장 덜 나쁜 곳으로 **걸어가서** 앉았다. 활동 중 플래너는
창을 40개 후보로 훑어 최고점을 고르므로 그쪽은 멀쩡했고, 그래서 "일할 때는 잘 되는데
아무것도 안 할 때 글자 옆에 앉는다"는 체감이 나왔다.

이제 두 가지가 다르다.

**여유 반경(`VisualEmptiness.clearance`).** 후보를 펫 프레임의 1배 · 1.5배 · 2배로 부풀려
차례로 채점하고, 0.55를 넘긴 단계 수가 그 후보의 등급이다. 0은 글자 위, 1은 글자 옆,
3은 사방에 펫 한 마리 폭이 비어 있는 자리다. 선택(`mostComfortable`)은 **3등급을 처음
만나면 그 자리에서 끝난다** — 그 정도 여유가 있는 자리는 서로 구분할 이유가 없고, 첫
번째를 잡아야 산책이 계산된 것처럼 보이지 않는다. 3등급이 없으면 최고 등급 중 첫 번째,
그것도 없으면 가장 덜 나쁜 것을 **`marginal`로 표시해서** 돌려준다.

**격자 훑기(`sweep`).** 캡처가 있을 때 director가 펫이 있는 디스플레이의 safe rect를
7×5 격자로 훑어 무작위 6개 **뒤에** 붙인다. 뒤에 붙이는 이유는 둘이다 — 무작위 후보가
같은 등급이면 그쪽이 이겨서 산책이 여전히 무작위로 보이고, 격자는 RNG를 한 번도 안
써서 캡처가 없을 때의 동작과 녹화된 세션이 한 바이트도 안 바뀐다.

**`marginal`이면 지금 자리를 본다.** 서 있는 자리가 0.55 이상이면 그 산책은 거절한다
(`.hold`, `parkedSince` 유지). 깨끗한 자리에서 글자 위로 걸어가는 것보다 안 걷는 편이
낫고, `isStrollDue`가 그대로라 다음 tick에 새 무작위 6개로 다시 묻는다. 서 있는 자리도
덮여 있으면(또는 잴 수 없으면) 예전처럼 덜 나쁜 곳으로 간다 — 거기서는 안 움직이는 것이
답이 아니다.

10'(escape)는 같은 선택기를 쓰되 `clear`만 받는다. 활동 중 플래너(`interest.rs`)는 손대지
않았다. 비용은 캡처 횟수 그대로에 채점만 후보 41개 × 최대 3회가 늘어난 것이고, 캡처
1회(62ms)와 비교하면 없는 값이다(`docs/battery.md`).

시나리오는 `gen-director.swift`의 `roam-clearance`에 박혀 있다 — 글자 옆(1등급) 대
가운데(3등급), 격자가 찾는 빈 구역, 깨끗한 자리에서의 거절, 덮인 자리에서의 덜 나쁜 선택.
Windows의 `cargo test`가 이 규칙을 보는 자리다.
