...
/// A tiny code-drawn cat used only when no licensed local package is present.
/// It deliberately follows the v2 grid so every runtime path remains active.
public enum PlaceholderPetFactory {
    ...
    public static func make() -> PetAsset {
        ...
        guard let context = CGContext(
            ...
            preconditionFailure("Unable to create placeholder pet bitmap")
        }

        context.clear(CGRect(x: 0, y: 0, width: width, height: height))
        context.setShouldAntialias(true)

        let usedFrames = [6, 8, 8, 4, 5, 8, 6, 6, 6, 8, 8]
        for row in 0..<rows {
        ...
        guard let atlas = context.makeImage() else {
            preconditionFailure("Unable to finish placeholder pet bitmap")
        }
