...
use persistence::{
    guide_seen, is_work_app, persist_work_app_label, remember, remember_guide, remember_tuning,
    stored_tuning,
};
...
use roamling_core::{
    ...
    InteractionOutput, PetAnimationPlayer, PetRuntime, TickInput, WorldPoint, WorldSize,
};
...
fn tick(hwnd: HWND, app: &mut App) {
    ...
    if output.persist_position {
        remember(&mut app.settings, output.position);
    }
