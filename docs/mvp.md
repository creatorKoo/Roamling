...
## 열려 있는 게이트는 없다 (2026-09-07 기준)

**MVP 사다리는 4에서 멈춘다.** 이후 게이트는 이 문서가 아니라 `docs/windows.md`의 W 사다리이고,
**그쪽도 전부 닫혔다** — W0~W3b(2026-09-01~02), 언어 결정 D(2026-09-02), W4~W7(2026-09-04),
W2b 디코더(2026-09-07). 양 플랫폼이 릴리스로 나가 있고 자동 업데이트가 켜져 있다.

그러므로 다음에 무엇을 할지는 **사다리가 정하지 않는다.** 실사용에서 나오는 것이 정한다 —
지금까지 그렇게 A~E 다섯 묶음과 결함 몇 개가 잡혔고, 그 목록은 `CLAUDE.md`의
"맥에서 해야 하는 일"에 있다.
...
**리팩터 게이트는 사용자가 달라진 점을 못 느껴야 통과한다.** W1·W2가 그렇게 닫혔고,
같은 규칙이 다음 리팩터에도 선다. 기존 테스트가 전부
