...
Roamling이 Codex 세션 기록에서 읽는 것이 하나 있고, 그것은 내용이 아닙니다. Codex의 훅은 승인
요청이 사용자에게 가는지 Codex의 자동 검토자에게 가는지 알려 주지 않습니다. 그래서 승인 요청이
오면 Roamling은 `~/.codex` 아래의 그 세션 기록을 열어, 턴을 여는 줄에서 설정값 하나
(`approvals_reviewer`)만 읽습니다. 같은 파일의 메시지·프롬프트·도구 출력은 해석하지 않고 건너뛰며,
아무것도 저장하지 않고, `~/.codex` 밖의 파일은 열지 않습니다. 검토자가 Codex 자신이면 보리는
사용자에게 묻지 않습니다 — 드물게 자동 검토자가 질문을 사용자에게 되돌릴 때에도 보리는 그것을
알지 못한다는 뜻이기도 합니다.
