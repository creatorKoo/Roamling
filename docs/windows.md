...
**"소스 보기"는 항목이고 macOS에서는 버튼이다.** `alert.about.body` 하나를
`MessageBoxW`로 띄우는데, 그 API는 버튼 이름을 바꿀 수 없다. 잘못 붙인 이름보다 한 칸
위의 항목이 낫다.
...
#### 맥에서 할 일 — 기본값 둘과 그 경계값 (2026-09-04)

사용자가 두 값을 **기본값으로 승격**해 달라고 했다. 실사용에서 그 값이 좋았기 때문이다.

```text
wanderPause      12   -> 40      경계  2 ~ 40      -> 2 ~ 78
hitRegionScale   1.12 -> 1.3     경계  0.75 ~ 1.3  -> 0.75 ~ 1.85
```

**경계값도 같이 넓혀야 한다.** 요청된 두 값이 각자 범위의 천장이라, 기본값만 바꾸면
슬라이더가 오른쪽 끝에 박혀 왼쪽으로만 움직인다 — 기본값이 트랙 중앙이라는 규칙과
정면으로 부딪친다. 위 표의 새 경계는 기본값 아래 여유만큼 위에도 준 값이다.
...
**`hitRegionScale` 상한을 1.85로 올리는 것은 동작 변경이다.** 펫의 클릭 영역이 그림보다
85% 커질 수 있게 된다 — "평소에는 아래 앱이 그대로 클릭된다"와 맞바꾸는 값이므로,
올릴 때 그 쪽을 한 번 확인할 것.
