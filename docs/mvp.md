...
## Current gate — W1: Runtime 추출 (`docs/windows.md`)
...
**W1은 사용자가 달라진 점을 못 느껴야 통과한다.** 기존 테스트가 전부 통과하고, 서명 빌드를
실사용해서 배회 · 포인터 회피 · 잡기 · 드래그 · agent 착석 · 취침 · 디스플레이 변경 ·
권한 프롬프트 · scale 변경이 전과 같아야 W2로 넘어간다. 리팩터 중에는 동작·타이밍·기본값을
같이 고치지 않는다 — 섞이면 회귀가 리팩터 탓인지 튜닝 탓인지 구분할 수 없다.
